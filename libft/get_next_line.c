...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/08/15 14:18:34 by celadia           #+#    #+#             */
/*   Updated: 2022/08/15 14:24:57 by celadia          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	gnl_fetch_fd(int fd, char **line, t_buf *buf)
{
	...
	while (true)
	{
		...
		if (newline)
		{
			*line = ft_substr(buf[fd].cur, 0, \
				newline - buf[fd].cur - (newline[-1] == '\r'));
			if (*line == NULL)
				return (gnl_terminate(-1, fd, line, buf));
...
void	buf_check(int fd, char *newline, t_buf *buf)
{
