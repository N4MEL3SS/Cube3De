...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/08/15 14:19:51 by celadia           #+#    #+#             */
/*   Updated: 2022/08/15 14:19:51 by celadia          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ray_intersect(t_game *g, double angle, unsigned int ray)
{
	...
	if (distance.x < distance.y)
		g->column[ray] = (struct s_column) \
			{(unsigned int)(g->col_scale / distance.x) & ~1, \
				x1.y - (int)x1.y, "EW"[x1.x < g->p.pos.x]};
	else
		g->column[ray] = (struct s_column) \
			{(unsigned int)(g->col_scale / distance.y) & ~1, \
